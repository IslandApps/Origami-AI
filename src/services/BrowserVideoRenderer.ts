...
// Re-exported so existing `from './services/BrowserVideoRenderer'` imports keep
// working; the bus itself now lives in ./videoEvents so the shared FFmpeg loader
// can publish to it without a circular import.
export { videoEvents };
...
export class BrowserVideoRenderer {
  ...
  async load() {
    ...
    // Delegates to the shared singleton in ./ffmpegLoader so the Shorts renderer
    // and this renderer share one wasm core. All CDN/multithread/progress
    // behaviour — including the 'FFmpeg ready' sentinel — lives there now.
  ...
  private async renderWithWebCodecs({
    ...
    const abortHandler = () => {
      ...
      this.loaded = false;
      this.ffmpeg = null;
      // A terminated core can never be reused — drop it from the shared cache so
      // the next load() rebuilds instead of handing back a dead instance.
      resetFFmpeg();
    };
