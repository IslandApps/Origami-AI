...
// Must match the backdrop's transition-duration in the markup below.
...
export function BackgroundDownloadToast({ active, queue }: BackgroundDownloadToastProps) {
  ...
  const showChrome = active && !dismissed;
  const dimmed = showChrome && !allReady;

  // Hold the page dim behind the toast for as long as resources are still
  // downloading; on completion, fade out and then drop the layer.
  useEffect(() => {
    if (dimmed) {
    ...
    const timer = setTimeout(() => setBackdropMounted(false), BACKDROP_FADE_MS);
  ...
  if (!showChrome && !backdropMounted) return null;
  ...
  return (
    <>
      {backdropMounted && (
        <div
          aria-hidden="true"
          className={`pointer-events-none fixed inset-0 z-[49] bg-black/45 motion-safe:transition-opacity motion-safe:duration-700 ${backdropVisible ? 'opacity-100' : 'opacity-0'}`}
        />
      )}
      {showChrome && (
        <div
